...
for trial in range(n_trial):
      ...
      for it in range(n_iteration):  
          ...
          g = batch_grad(index_set, n_data, n_feature, X, y, w, r)    
          
          ################################
          m = beta*m + (1 - beta)*g
          v = theta*v + (1 - theta)*g*g
          v_ams = np.maximum(v_ams, v)
          eta = alpha/np.sqrt(v_ams + eps)
          w = w - eta*m
          ################################
         
          eta_hat = 1/np.sqrt(v_ams + eps)
